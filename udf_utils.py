...
def extract_start_date(file_content):
    try:
        ...
        start_date = datetime.strftime(opendate_match.group(2), '%m-%d-%y') if opendate_match else None
        return start_date
    except Exception as e:
...
def extract_end_date(file_content):
        ...
        enddate = datetime.strftime(enddate, '%B %d,%Y') if enddate else None
        return enddate
...
def extract_salary(file_content):
    try:
        ...
        if salary_match:
            ...
            salary_end = float(salary_match.group(4).replace(',', ' ')) if salary_match.group(4) else float(salary_match.group(2).replace(',', ' '))
        else:
...
def extract_notes(file_content):
    try:
        notes_match = re.search(r'(NOTES?):(.*?)(?=DUTES)', file_content,
...
def extract_experience_length(file_content):
    try:
        experience_length_match = re.search(
            r'(One|Two|Three|Four|Five|Six|Seven|Eight|Nine|Ten|one|two|three|four|five|six|seven)\s(years?)\s(of\sfull(-|\s)time)', file_content
        )
...
def extract_education_length(file_content):
    try:
        education_length_match = re.search(
            r'(One|Two|Three|Four|Five|Six|Seven|Eight|Nine|Ten|one|two|three|four|five|six|seven)(\s|-)(years?)\s(college|university)', file_content
        )
